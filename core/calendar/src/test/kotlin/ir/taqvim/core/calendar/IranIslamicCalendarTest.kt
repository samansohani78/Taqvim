...
/** A-05 against the official calendars of 1404 and 1405 (golden/persian and golden/islamic-iran). */
class IranIslamicCalendarTest {
    ...
    @Test
    fun `the tabular estimate meets the table at both edges`() {
        val firstOfficial = iran.toJdn(hijri(1446, 9, 1))
        val firstAfter = iran.toJdn(hijri(1448, 10, 1))

        val lastDayOfShaban = TabularIslamicCalendar.TYPE_II.monthLength(1446, 8)
        iran.fromJdn(Jdn(firstOfficial.value - 1)) shouldBe hijri(1446, 8, lastDayOfShaban)
    ...
    fun `100 000 random days round-trip on both sides of the table`() {
        ...
        (1..RANDOM_DAYS)
            .map { Jdn(random.nextLong(1_948_440L, 2_600_000L)) }
