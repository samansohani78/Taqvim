...
Usage: tools/iran/official_eclipses_golden.py [--check] (run from anywhere; paths are relative to the repository root)

Sources are the owner-supplied Calendar Center documents in docs/sources, each checked against the SHA-256 in
docs/sources/MANIFEST.md before use:
...
CALENDAR_1404 = "docs/sources/Calendar-1404.pdf"
CALENDAR_1405 = "docs/sources/Calendar-1405.pdf"
NOTICE = "docs/sources/اطلاعیه ماه گرفتگی 16 شهریور 1404.pdf"
...
def manifest_digest(name):
    ...
    raise SystemExit(f"{name}: no SHA-256 row in {MANIFEST}")
...
def eclipses_csv(rows, digests):
    ...
    header = [
        ...
        "page 2 also keeps a section for 1399; generated by tools/iran/official_eclipses_golden.py with pdftotext "
        "-raw, not typed",
        ",".join(columns),
    ]
...
def sites_csv(rows):
    header = [
        ...
        "re-checked against docs/sources/MANIFEST.md); degrees north and east; generated by "
        "tools/iran/official_eclipses_golden.py, not typed",
...
def main():
    ...
    if "--check" in sys.argv[1:]:
        ...
        if stale:
            raise SystemExit(f"out of date: {', '.join(stale)}; rerun tools/iran/official_eclipses_golden.py")
        print("up to date")
        return
