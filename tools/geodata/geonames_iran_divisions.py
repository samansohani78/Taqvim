...
Reads the two per-country GeoNames extracts for Iran (each a zip holding one 'IR.txt'):
  ...
  - the alternate-names dump (https://download.geonames.org/export/dump/alternatenames/IR.zip), from which the
    ...
    Tamil (ta), Tajik (tg), Uzbek (uz), Malay (ms) and Nepali (ne) — the same per-country dump already carries every
    language's alternate names for Iran's geonameids, not just Persian's, so no extra download is needed for these.
...
empty field for it, the same convention cities.tsv uses for a name equal to the English one. Coverage of the 8
DT-020 languages is sparse and almost entirely limited to the 31 provinces (checked 2026-09-25: between 30 and 32 of
the 466 divisions have a name in each of ps/ckb/az/uz/ms/tg/ta, and only 1 has one in ne) — Kurmanji (`kmr`) and Dari
(`prs`) are checked and NOT written: GeoNames' `kmr` and `prs` tags exist but neither has a single alternate name for
any Iran division in this dump (checked 2026-09-25).
"""
import hashlib
import io
import sys
...
# DT-020: Pashto, Central Kurdish, Azerbaijani, Uzbek, Malay, Tajik, Nepali, Tamil, grouped by script like cities.tsv.
DT_020_LANGUAGES = ["ps", "ckb", "az", "uz", "ms", "tg", "ne", "ta"]
...
def read_localized_names(text, geoname_ids, languages):
    ...
    for line in text.splitlines():
        ...
        if language not in languages or geoname_id not in geoname_ids:
            continue
        name = clean(name)
        if not name:
            continue
