...
- **Golden (2026-09-25, DT-014):** `core/astronomy/src/test/resources/golden/panchang/tithi-1945se.csv` — the
  Rashtriya Panchang (English), Saka Era 1945 (2023–24 A.D.), published by the Positional Astronomy Centre of the
  India Meteorological Department, Ministry of Earth Sciences, retrieved from the Internet Archive's capture of
  packolkata.gov.in (`.../download/EnglishRP2324.zip`, SHA-256 `38c50c8d…8522`; `RP_1945SE.pdf`, SHA-256
  `9ba9dd39…57c4`) and generated by `tools/sources/india/rashtriya_panchang_golden.py`. It is a **priced** Government
  of India publication (Rs. 225), so the PDF is cited rather than archived and only the factual instants are kept.
  Comparable because both sides are the same quantity: the Panchang is computed "on modern scientific principles"
  from the Indian Astronomical Ephemeris and states its tithi moments are geocentric and so "require no correction
  for other places", exactly as `Tithi` takes true geocentric longitudes. Over the **376** printed ending moments of
  the year, `Tithi.changes` is never earlier than the printed minute and at most 95 s later (median 50 s) — the shape
  truncation to the printed minute predicts. This does **not** check `NepaliLunarDays`, which uses a Surya Siddhanta
  Moon on purpose (ADR-0030) and differs by hours.
- **Golden (2026-09-25):** `core/astronomy/src/test/resources/golden/usno/twilight-2026.csv` — U.S. Naval Observatory
  API v4.0.1 `/api/rstt/oneday`, the 21st of each month of 2026 for Tehran, Kabul, Istanbul, Berlin and Sydney (the
  cities DT-011 names), archived raw in `docs/sources/usno/twilight-2026-raw.json` and generated by
  `tools/usno/usno_twilight_golden.py`. US government work, public domain (17 U.S.C. 105). Rise, set and transit agree
  within the ±3 min acceptance criterion on all 60 days. The blue hour does **not** coincide with USNO civil twilight:
  USNO defines it at a geometric −6°, while `PhotographyPanel.BLUE_HOUR_BOTTOM` is an apparent altitude and
  `Refraction.Normal` clamps the correction below the horizon, so the app's −6° apparent is about −6.6° geometric and
...
- **Files:** `dataset/afghanistan/afghanistan-official-holidays.json` (10 records); generated
...
- **Second source (2026-09-25, DT-032): the Labour Law itself.** The announcements say *which* days are off in a
  given year; the law says what the holidays *are*. Afghanistan's Labour Law (No. 35 of 2007), Official Gazette
  No. 966, is published in Dari by the Ministry of Labour and Social Affairs at
  https://molsa.gov.af/sites/default/files/2019-06/قانون%20کار%20966.pdf (149 pp, SHA-256
  `9ec90caa871c6ad537f7e0073bede5b2e1e6f0e84afe0bb59eb2022ec1b8686c`; its text layer is a custom encoding that does
  not extract), and in English translation by the ILO as NATLEX record 78309,
  https://natlex.ilo.org/dyn/natlex2/natlex2/files/download/78309/AFG78309.pdf (36 pp, SHA-256
  `28cb8ba93a578d1ac4c52e97de11b722a6034957f488c87f2efcee59360ead22`). Both cited, neither copied. **Article 41**
  lists eleven public holidays: Friday; Nawruz; 28 Asad; 8 Sawr; 1 Ramadan; «The three days of Eid-e-Feter»; «Four
  days of Eid-e-Said-e-Adhah and Arafat (Three days of Eid and one day of Arafat)»; 12 Rabi al-Awwal; 10 Muharram;
  26 Dalw; and any other announced day.
- **What was taken from it, and what was not.** Only the two Eid spans: Eid al-Fitr 1–3 Shawwal (three new records —
  the app previously had **no Afghan Eid al-Fitr at all**, because the 2026-03-21 announcement names the holiday
  without dates) and 12 Dhu al-Hijjah, the third day of Eid al-Adha. The 1447 announcement and Article 41 agree once
  the difference in what each counts is seen: the announcement gives the four *working* days off (9, 10, 11 and 13,
  since 12 Dhu al-Hijjah 1447 fell on a Friday), while Article 41 fixes the span at Arafa plus three Eid days, 9–12.
...
- **Files:** `data/location/src/main/resources/ir/taqvim/data/location/cities.tsv` (7342 places), generated by
  `tools/geodata/natural_earth_cities.py`, then `tools/geodata/geonames_place_names.py` (DT-020).
...
- **Files:** `data/location/src/main/resources/ir/taqvim/data/location/iran-divisions.tsv` (31 provinces, 435
  counties), generated by `tools/geodata/geonames_iran_divisions.py`; read by `IranDivision.kt`,
  `IranDivisionTableParser.kt`, `IranDivisionCatalog.kt`.
- **Source:** GeoNames gazetteer, Iran (`IR`) country dump, retrieved 2026-09-25 from
  https://download.geonames.org/export/dump/IR.zip (SHA-256
  `a508c1962f00e637217e42b114d9dec1187abe6db73a9db56731f60dc0d9541f`) — the `ADM1` (province) and `ADM2`
  (county/shahrestan) feature rows, each with its own name and point coordinates. Persian (`fa`) names from the
  matching per-country alternate-names dump, retrieved the same day from
  https://download.geonames.org/export/dump/alternatenames/IR.zip (SHA-256
  `52a2f2c70206baa69bc3c9fc65c0426751affb501d3042ff384c9fa20ee19b8e`): the alternate name flagged
...
- **Checked, not used:** Statistical Centre of Iran (amar.org.ir) — reachable over plain HTTP but its HTTPS
  ...
  from it. No reachable Ministry of Interior country-divisions publication was found.
- **Licence:** CC BY 4.0 (https://creativecommons.org/licenses/by/4.0/; https://www.geonames.org/about.html),
  admitted for bundled data by ADR-0039 (`CC-BY-4.0` already in `config/license/allowed-licenses.json`
  `dataLicenses` for the Matthews plate layer; no new allow-list entry needed). Required attribution: GeoNames,
  2026, "GeoNames geographical database", https://www.geonames.org/, CC BY 4.0 — shown in About > Data sources
  since `cities.tsv` (a screen the app already reads) also carries GeoNames-sourced names as of this same pass; see
  `about_source_geonames_attribution`.
- **Transformation:** kept the province's admin1 code (e.g. `26`) as its `code`, and each county's own geonameid as
  its `code` with the parent province's admin1 code as `parentCode`; English name and coordinates copied as
  published, rounded to 5 decimals. A missing name in a given language is left empty, the same convention
  `cities.tsv` uses. Nothing translated, corrected or added.
...
### core/i18n — Pashto era abbreviations (`FormatTable.PRODUCT_ERAS`, DT-008)
- **Source:** *رسمي جریده* (Rasmi Jarida), the Official Gazette of Afghanistan, published by its Ministry of Justice.
  The cover of each issue carries the publication date twice, once in Pashto (`د خپرېدو نېټه`) and once in Dari
  (`تاریخ نشر`), with the year marked by an era abbreviation. Read from the page images and text layers of seven
  issues, retrieved 2026-09-26:
  - serial 1424, 14 لړم 1401 هـ.ش / 10 ربیع الثاني 1444 هـ.ق (Islamic Emirate), the only issue read that dates in both
    calendars: `د ۱۴۴۴ هـ.ق کال د ربیع الثاني د میاشتې (۱۰)` / `د ۱۴۰۱ هـ.ش کال د لړم د میاشتې (۱۴)`, and in Dari
    `تاریخ نشر: (۱۰) ربیع الثانی سال ۱۴۴۴ هـ.ق` / `(۱۴) عقرب سال ۱۴۰۱ هـ.ش`. Linked from
    https://moj.gov.af/en/node/1515 as `old.moj.gov.af/Content/files/OfficialGazette/01401/OG_01424.pdf`, which no
    longer serves TLS; read through the Internet Archive capture
    https://web.archive.org/web/20221124045802id_/http://old.moj.gov.af/Content/files/OfficialGazette/01401/OG_01424.pdf
  - serials 1306, 1310, 1329 and 1334 (all 1397 هـ.ش) and 1150 (1393 هـ.ش), Islamic Republic, each
    `د خپرېدو نېټه: د <year> هـ.ش کال د <month> د میاشتې (<day>)` and `تاریخ نشر: (<day>) <month> سال <year> هـ.ش`,
...
- **Variant not used:** the Ministry's own Pashto web page (https://moj.gov.af/ps/رسمي-جريده, retrieved 2026-09-26)
  writes one date as `د۱۳۹۷هـ ل کال` (`ل` for لمریز, "solar"). One web sentence against seven printed covers spanning
  1393–1401, so the gazette's `هـ.ش` is taken and the variant recorded here.
- **Author / date:** Saman Sohani (via Claude Code), 2026-09-26; **reviewer:** pending.
...
- **Names and pattern (main@1a08566, ADR-0030 addendum; `core/i18n/.../bikram-sambat.properties`):** Nepali month names
  ...
  (DT-010, resolved 2026-09-25) is `d MMMM, y`, read from two Government of Nepal sources showing an actual written
  date: the Nepal Law Commission's statute pages (lawcommission.gov.np, "१३ माघ, २०८१", retrieved 2026-09-25) and the
  Department of Printing's Nepal Gazette upload listing (dop.gov.np, "८ असोज, २०८३", retrieved 2026-09-25) — both
  giving "day, full month name, comma, year" with no era or weekday in the date string itself.
- **Not consulted:** any nepali-date library, BS month-length table from GitHub, npm or Wikipedia, or GPL/LGPL code.
- **Author / date:** Saman Sohani (via Claude Code), 2026-09-16, pattern added 2026-09-25; **reviewer attestation:**
  pending — no forbidden sources consulted.

### F07 — Hebrew month names (`core/i18n` `hebrew-months.properties`, T-108, ADR-0025 addendum)
- **Source:** Unicode CLDR 48 Hebrew-calendar standalone wide month names, read through the ICU4J 78.3 public API
  (Unicode-3.0 licence, notice in `licenses/`), retrieved 2026-09-16 — the same approach as the T-200 language table.
  Generated by `tools/i18n/HebrewMonthsGen.java` (do not edit the properties file by hand) and re-checked against ICU4J
  by `HebrewMonthNamesTest` on every run; main@381f6c0.
- **Coverage:** real names for fa, prs (fa-AF), ar, tr, ur, ta, bn, en, ru, de, fr, es, ja. ps, ckb, kmr, az, ne, hi, tg,
  uz, id and ms only inherit the root names and zh has generic numbered months, so they are omitted (DT-037).
- **Mapping:** `HebrewMonthNames` numbers months from Tishri like `HebrewCalendar` (12 in a common year, 13 with Adar I
  and Adar II in a leap year).
...
- **Coverage:** one Latin name per constellation, used as the universal fallback for all 24 launch languages (IAU
  ...
  No CLDR data exists for constellation names at any release (confirmed against ICU4J 78.3/CLDR 48).
...
#### Per-language names (`iau-constellations-i18n.properties`), resolved 2026-09-25 for Japanese
- **Source (`ja`):** National Astronomical Observatory of Japan (国立天文台, NAOJ; part of the National Institutes of
  Natural Sciences), "星座名・星座略符一覧(星座名順)" ("List of constellation names and abbreviations, by name"),
  https://www.nao.ac.jp/new-info/constellation.html, retrieved 2026-09-25 (SHA-256 of the fetched, Shift-JIS-encoded
  page `859a737b66f2ae380739c39762d25e134bee676030ad0689d693c566f96358db`).
- **Extraction:** parsed directly from the page's own HTML table with a script, not a model summary — a first,
  model-summarized fetch of the same page produced two rows for a duplicated abbreviation (Tuc/Pav), a transcription
  error only caught by re-fetching the raw HTML and parsing its `<table>` structurally. All 88 abbreviations matched
  `IauConstellationNames`' key set exactly, including Ser (Serpens), whose row uses `rowspan="2"` for its two
  non-contiguous halves (head/tail) and needed a separate cell-class pattern (`name4`/`name5`/`name8` rather than
  `name`/`name2`) to extract.
- **Transformation:** the table's "name" column gives each constellation's reading in hiragana/katakana, with an
  optional parenthetical kanji gloss and, for the 12 zodiacal constellations, a "＊" marker; the page's own
  explanatory text states names are written in hiragana or katakana and kanji is shown only so the meaning is clear.
  The kanji gloss and "＊" marker were dropped and "座" ("-za", the Japanese word-forming suffix for "constellation")
  appended to the reading, matching the page's own usage of the suffixed form in running text (its own worked
  example, "オリオン座α星" for "α Orionis"). Nothing translated or invented: every character comes from the page.
- **Licence:** LicenseRef-Cited-Publication (config/license/allowed-licenses.json) — a name-and-abbreviation fact,
  not a licensed creative work; no attribution required, this citation is the record.
- **Not sourced this pass, checked (docs/DATA_TODO.md DT-026):** Chinese (`zh`) — 《天文学名词》(third edition, 2024),
  the government-approved terminology of 全国科学技术名词审定委员会 (National Committee for Terms in Sciences and
  Technology), is digitized at https://nadc.china-vo.org/astrodict/ (National Astronomical Data Center / National
  Astronomical Observatories, Chinese Academy of Sciences) and one spot-checked entry matched (Gemini/双子座), but the
  site is a term-by-term dictionary with no found category-browse URL, so a full 88/88 enumeration was not completed
  or verified. Russian (`ru`) — astronet.ru lists all 88 Latin↔Russian pairs, but its current institutional backing
  could not be confirmed from the fetched page alone (no visible licence statement). Persian (`fa`) — the Academy of
  Persian Language and Literature (apll.ir) and its terminology search tools were checked; no dedicated
  constellation-name list was found. Arabic, Turkish, German, French, Spanish, Hindi and the remaining 17 languages
  were not found to have a citable national-authority 88-constellation list this pass (a few, e.g. Turkish or
  Arabic astronomical bodies, were spot-checked and had no indexed list; most of the smaller languages — ps, prs,
  ckb, kmr, az, ne, ta, tg, uz — were not searched this pass, deprioritized as least likely to have one).
- **Author / date:** Saman Sohani (via Claude Code), 2026-09-25; **reviewer attestation:** pending — no forbidden
  sources consulted.
