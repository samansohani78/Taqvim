...
- **Source:** GeoNames gazetteer, Iran (`IR`) country dump, retrieved 2026-09-25 from
  ...
  `isPreferredName`, or the first one found, per geonameid.
...
- **Licence:** CC BY 4.0 (https://creativecommons.org/licenses/by/4.0/; https://www.geonames.org/about.html),
  ...
  2026, "GeoNames geographical database", https://www.geonames.org/, CC BY 4.0, no modification to the names or
  coordinates beyond rounding coordinates to 5 decimals — not yet shown in the app (no About > Data sources entry
  or `DataSource` enum case added in this pass, since no screen reads this file yet; T-1502's district picker is
  still unbuilt (docs/DATA_TODO.md DT-021) and should add both when it lands).
- **Transformation:** kept the province's admin1 code (e.g. `26`) as its `code`, and each county's own geonameid as
  ...
  published, rounded to 5 decimals. A missing Persian name (3 of 435 counties) is left empty, the same convention
...
- **Author / date:** Saman Sohani (via Claude Code), 2026-09-25
- **Reviewer attestation:** pending — no forbidden sources consulted.
