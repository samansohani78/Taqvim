...
| Item | Location | Currently | Can it be computed? | Action |
|---|---|---|---|---|
| City catalog (7 342 places, zones) | `data/location/src/main/resources/.../cities.tsv` | bundled table (Natural Earth, public domain; place names in 8 more languages from GeoNames, CC BY 4.0, DT-020) | no — geography | None per year. Refresh only if a newer Natural Earth release or GeoNames extract is wanted. Without it: coordinates entry and GPS still work. DT-020: `kmr` and `prs` names not added, GeoNames' coverage of both is too sparse (docs/DATA_TODO.md). |
