...
| Item | Location | Currently | Can it be computed? | Action |
|---|---|---|---|---|
| City catalog (7 342 places, zones) | `data/location/src/main/resources/.../cities.tsv` | bundled table (Natural Earth, public domain; place names in 9 more languages from GeoNames, CC BY 4.0, DT-020) | no — geography | None per year. Refresh only if a newer Natural Earth release or GeoNames extract is wanted. Without it: coordinates entry and GPS still work. DT-020: `prs` names not added — GeoNames has 6 Dari-tagged names worldwide and no `fa-AF` tag, and Dari writes these names as Iranian Persian does, so `City` falls back to the `fa` name (docs/DATA_TODO.md). |
| Iran country divisions (31 provinces, 435 counties) | `data/location/src/main/resources/.../iran-divisions.tsv` (DT-021, DT-020) | bundled table (GeoNames, CC BY 4.0) | no — geography | None per year. Refresh only if a newer GeoNames extract is wanted. District (bakhsh) level not bundled: no source with adequate coverage found (DT-021). Read by the city/location screens via `cities.tsv`'s attribution; T-1502's district picker still unbuilt. |
...
- [x] REVIEW R14 (2026-09-19) — the completeness claims narrowed to what the tests assert, and the tests tightened: day assembly runs under the defaults of all 24 launch languages (was 5); the source-aware Hijri date is required exactly for the Iranian official variant and must equal the day's Islamic date (a null was accepted before); prayer times are checked in chronological order with midnight crossing handled (Tromsø only checked that four values were in 0…1439); Placidus houses are required at Tehran and must be absent inside the polar circle. Required and optional fields are listed per place and state in each test's documentation. Tithi dates stay checked against the app's own routine only (DT-014), so a systematic tithi error cannot be found. **Found and left open for `:core:praytimes`:** at Tromsø, Asr after sunset on 10 days of 2001–2101 (e.g. 2001-11-27: Asr 11:40, sunset 11:39) and maghrib after isha on 2 889 days (e.g. 2001-04-19: maghrib 22:06, isha 22:04); both are allowed by name in `KNOWN_POLAR_ORDER_DEFECTS` so every other ordering stays enforced.
