...
| Iran country divisions (31 provinces, 435 counties) | `data/location/src/main/resources/.../iran-divisions.tsv` (DT-021) | bundled table (GeoNames, CC BY 4.0) | no — geography | None per year. Refresh only if a newer GeoNames extract is wanted. District (bakhsh) level not bundled: no source with adequate coverage found (DT-021). Not yet read by any screen (T-1502 unbuilt). |
