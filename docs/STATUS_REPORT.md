...
| T-1600 Wear device checks | Done and passing (main@62b0524): the `wearApi34` suite runs on a Wear OS 34 emulator job in CI and locally, 11/11 twice. Four test defects were behind the earlier red job — unanchored `By.res` matching nested screen tags, no watch node reporting `scrollable=true`, taps missing on the morphing list edges, and swipe-to-dismiss animating back | Only watch-face tile and complication placement stays manual (`docs/MANUAL_TEST_CHECKLIST.md` §7) |
